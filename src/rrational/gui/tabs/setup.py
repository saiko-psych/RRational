...
def _render_events_section():
    ...
    st.info(
        "All event matching is done in **lowercase** automatically to reduce the number of synonyms needed."
    )

    # Create new event
    with st.expander("Create New Event"):
        new_event_name = st.text_input(
            "Event Name (canonical)", key="new_event_name_global"
        )
        ...
        def create_event():
            ...
            if event_name and event_name not in st.session_state.all_events:
                synonyms_list = [
                    s.strip().lower() for s in synonyms_raw.split("\n") if s.strip()
                ]
    ...
    def _rename_event(old_name: str):
        ...
        if new_name != old_name and new_name not in st.session_state.all_events:
            st.session_state.all_events[new_name] = st.session_state.all_events.pop(
                old_name
            )
            for group_data in st.session_state.groups.values():
                if old_name in group_data.get("expected_events", {}):
                    group_data["expected_events"][new_name] = group_data[
                        "expected_events"
                    ].pop(old_name)
            auto_save_config()
            update_normalizer()
    ...
    if st.session_state.all_events:
        for event_name, synonyms in list(st.session_state.all_events.items()):
            with st.expander(
                f"Event: {event_name} ({len(synonyms)} synonym(s))", expanded=False
            ):
                # Editable event name
                ...
                used_in_groups = [
                    gname
                    for gname, gdata in st.session_state.groups.items()
                ...
                with col2:
                    ...
                    st.button(
                        ...
                        args=(event_name,),
                        type="primary",
                        disabled=not new_synonym
                        or validate_regex_pattern(new_synonym.strip()) is not None,
                    )
...
def _render_groups_section():
    ...
    st.markdown(
        "Create groups, edit/rename/delete them, and assign events from the Event Mapping tab."
    )
    ...
    with st.expander("Create New Group"):
        ...
        new_group_label = st.text_input(
            "Group Label (display name)", key="new_group_label"
        )
    ...
    for group_name, group_data in list(st.session_state.groups.items()):
        with st.expander(
            f"{group_name} - {group_data['label']}", expanded=(group_name == "Default")
        ):
            ...
            def save_group_changes(old_name):
                ...
                new_name_val = st.session_state.get(
                    f"edit_group_name_{old_name}", old_name
                )
                new_label_val = st.session_state.get(
                    f"edit_group_label_{old_name}", old_name
                )

                current_name = old_name
                if new_name_val != old_name:
                    st.session_state.groups[new_name_val] = st.session_state.groups.pop(
                        old_name
                    )
            ...
            participant_count = sum(
                1
                for g in st.session_state.participant_groups.values()
                if g == group_name
            )
            ...
            if available_sections:
                ...
                def update_sections(grp_name):
                    ...
                    st.session_state.groups[grp_name]["selected_sections"] = (
                        st.session_state[f"sections_select_{grp_name}"]
                    )
                    auto_save_config()
                ...
                st.info(
                    "No sections defined yet. Create sections in the Sections tab first."
                )
            ...
            st.markdown(
                "*Click events to add/remove from this group (saves automatically):*"
            )
            ...
            for idx, event_name in enumerate(available_event_names):
                ...
                with cols[col_idx]:
                    ...
                    def toggle_event(grp_name, evt_name, currently_selected):
                        ...
                        exp_events = st.session_state.groups[grp_name][
                            "expected_events"
                        ]
                        if st.session_state[f"event_select_{grp_name}_{evt_name}"]:
                            if evt_name not in exp_events:
                                exp_events[evt_name] = st.session_state.all_events[
                                    evt_name
                                ].copy()
                                st.session_state.groups[grp_name]["expected_events"] = (
                                    exp_events
                                ...
                                auto_save_config()
                                show_toast(
                                    f"Added '{evt_name}' to {grp_name}", icon="success"
                                )
                        else:
                            if evt_name in exp_events:
                                ...
                                auto_save_config()
                                show_toast(
                                    f"Removed '{evt_name}' from {grp_name}", icon="info"
                                )
            ...
            def delete_group(grp_name):
                ...
                auto_save_config()
                show_toast(
                    f"Deleted group '{grp_name}' and reassigned participants to Default",
                    icon="success",
                )
    ...
    st.markdown("---")
    st.info(
        "**All changes save automatically** when you modify group settings or select events."
    )
...
def _render_event_sequences_section():
    ...
    with st.expander("Create New Event Sequence"):
        new_seq_name = st.text_input(
            "Sequence ID (e.g., sequence_01)", key="new_sequence_name"
        )
        ...
        def create_event_sequence():
            if new_seq_name and new_seq_name not in st.session_state.event_sequences:
                ...
                show_toast(
                    f"Event sequence '{new_seq_name}' already exists", icon="error"
                )

        st.button(
            "Create Event Sequence",
            key="create_sequence_btn",
            on_click=create_event_sequence,
        )
    ...
    if not st.session_state.event_sequences:
        ...
        for seq_name, seq_data in list(st.session_state.event_sequences.items()):
            condition_order = seq_data.get(
                "condition_order", seq_data.get("music_order", [])
            )
            with st.expander(f"{seq_name} - {seq_data.get('label', seq_name)}"):
                ...
                with col1:
                    ...
                    def save_sequence_changes(s_name, new_ord, new_lbl):
                        order_list = [
                            m.strip() for m in new_ord.split(",") if m.strip()
                        ]
                        if order_list:
                            st.session_state.event_sequences[s_name][
                                "condition_order"
                            ] = order_list
                ...
                with col3:
                    ...
                    def delete_sequence(s_name):
                        ...
                        for pid in list(st.session_state.participant_sequences.keys()):
                            if (
                                st.session_state.participant_sequences.get(pid)
                                == s_name
                            ):
                        ...
                        for pid in list(
                            ...
                            if (
                                st.session_state.participant_randomizations.get(pid)
                                == s_name
                            ):
                ...
                participants_in_group = [
                    pid
                    for pid, s in st.session_state.get(
                        "participant_randomizations", {}
                    ).items()
                    if s == seq_name
                ]
    ...
    for s_data in st.session_state.get("event_sequences", {}).values():
        all_conditions.update(
            s_data.get("condition_order", s_data.get("music_order", []))
        )
    ...
    if all_conditions:
        ...
        edited_labels = st.data_editor(
            ...
            column_config={
                "Code": st.column_config.TextColumn(
                    "Code", disabled=True, help="Internal identifier"
                ),
                "Label": st.column_config.TextColumn(
                    "Label", help="Short display name"
                ),
                "Description": st.column_config.TextColumn(
                    "Description", help="Full description", width="large"
                ),
            },
        )
        ...
        st.button(
            ...
            key="save_condition_labels_btn",
            on_click=save_condition_labels_callback,
            type="primary",
        )
    else:
    ...
    st.markdown("---")
    st.info(
        "**All changes save automatically.** Condition labels are used in the Data tab."
    )
...
def _render_sections_section():
    ...
    if "sections" not in st.session_state:
        loaded_sections = load_sections(st.session_state.get("current_project"))
        if not loaded_sections:
            ...
            st.session_state.sections = {
                ...
                "pre_pause": {
                    "label": "Pre-Pause",
                    "description": "Music before pause",
                    "start_events": ["measurement_start"],
                    "end_events": ["pause_start"],
                    "expected_duration_min": 90.0,
                    "tolerance_min": 5.0,
                },
                "post_pause": {
                    "label": "Post-Pause",
                    "description": "Music after pause",
                    "start_events": ["pause_end"],
    ...
    with st.expander("Create New Section"):
        ...
        with st.form(
            "create_section_form", clear_on_submit=True, enter_to_submit=False
        ):
            new_section_name = st.text_input(
                "Section Code (internal ID)", help="e.g., music_01, rest_pre"
            )
            new_section_label = st.text_input(
                "Section Label (short name)", help="e.g., Music 1, Pre-Rest"
            )
            new_section_desc = st.text_input(
                "Description (detailed)", help="e.g., Brandenburg Concerto No. 3 - Bach"
            )
            ...
            if submitted:
                if (
                    new_section_name
                    and new_section_name not in st.session_state.sections
                ):
                    if not start_events:
    ...
    if st.session_state.sections:
        ...
        edited_sections = st.data_editor(
            ...
            column_config={
                "Code": st.column_config.TextColumn(
                    "Code", help="Internal identifier", width="small"
                ),
                "Label": st.column_config.TextColumn(
                    "Label", help="Short display name", width="medium"
                ),
                "Start Event(s)": st.column_config.TextColumn(
                    "Start Event(s)",
                    help="Comma-separated list of events (any can start section)",
                    width="medium",
                ),
                "End Event(s)": st.column_config.TextColumn(
                    "End Event(s)",
                    help="Comma-separated list of events (any can end section)",
                    width="medium",
                ),
                ...
                "Tolerance (min)": st.column_config.NumberColumn(
                    "Tolerance (min)",
        ...
        if st.button("Save Section Changes", key="save_sections_btn", type="primary"):
            ...
            for _, row in edited_sections.iterrows():
                ...
                if section_code:  # Skip empty rows
                    ...
                    if not isinstance(start_events_str, str) or pd.isna(
                        start_events_str
                    ):
                        start_events_str = ""
                    start_events_list = [
                        e.strip() for e in start_events_str.split(",") if e.strip()
                    ]
                    ...
                    invalid_start = [
                        e for e in start_events_list if e not in valid_events
                    ]
                    if invalid_start:
                        validation_errors.append(
                            f"Section '{section_code}': invalid start event(s): {', '.join(invalid_start)}"
                        )
                    ...
                    if not isinstance(end_events_str, str) or pd.isna(end_events_str):
                        end_events_str = ""
                    end_events_list = [
                        e.strip() for e in end_events_str.split(",") if e.strip()
                    ]
                    ...
                    if invalid_end:
                        validation_errors.append(
                            f"Section '{section_code}': invalid end event(s): {', '.join(invalid_end)}"
                        )
            ...
            if validation_errors:
                for err in validation_errors:
                    st.error(err)
                st.warning(
                    "Define missing events in the **Events** tab first, or correct the event names."
                )
            else:
            ...
            codebook_data.append(
                {
                    ...
                    "Description": f"Synonyms: {', '.join(synonyms[:3])}"
                    if synonyms
                    else "",
                }
            )
