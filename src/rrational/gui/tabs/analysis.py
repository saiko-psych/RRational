    ...
    with tab_chart:
        ...
        if viz_type == "SD1/SD2 Scatter":
            ...
            if not available_metrics:
                ...
                with col3:
                    # Default: log-y auto-enabled for known log-normal metrics
                    from rrational.analysis.group_statistics import (
                        should_log_transform,
                    )

                    default_log = should_log_transform(selected_chart_metric)
                    log_y = st.checkbox(
                        "Log Y-axis",
                        value=default_log,
                        key=f"group_log_y_{selected_chart_metric}",
                        help=(
                            "Use logarithmic y-axis. Recommended for "
                            "frequency-domain metrics (LF, HF, VLF) which are "
                            "typically log-normally distributed."
                        ),
                    )

                # Section filter + show individual points toggle
                col_sec, col_points = st.columns([3, 1])
                with col_sec:
                ...
                with col_points:
                    show_points = st.checkbox(
                        ...
                        value=False,
                        key="group_analysis_show_points",
                        help=(
                            "Overlay each participant's value as a point on "
                            "the bar chart. Reveals the actual distribution "
                ...
                if not chart_sections:
                    ...
                    if viz_type == "Bar Chart":
                        fig = _create_group_bar_chart(
                            ...
                            long_df=filtered_df if show_points else None,
